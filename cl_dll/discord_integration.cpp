...
void GetGameInfo(gameinf_s* inf);
...
namespace discord_integration
{
	namespace
	{
		...
		constexpr const char CLIENT_ID[] = "840176948856356874";
		...
		public:
			...
			inline void set_start_time(int new_start_time)
			{
				...
				if (cur_state != game_state::NOT_PLAYING)
				{
					...
					else
					{
						if (gameinf.chaptertitles)
							presence.details = gameinf.chpname.c_str();
					}
	...
	{
		discord_state->set_start_time(start_time);
	}
...
// BlueNightHawk : Get chapter title and the chapter's image's file name
void GetGameInfo(gameinf_s* inf)
{
	if (IsDeathmatch())
	{
		inf->chaptertitles = 0;
		inf->gamemode = "Deathmatch";
		return;
	}

	char mapname[64];
	get_map_name(mapname, ARRAYSIZE(mapname));

	if (!strncmp(mapname, "t0", 2))
	{
		inf->chpname = "Hazard Course";
		inf->thumbname = "hc";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c0", 2))
	{
		inf->chpname = "Black Mesa Inbound";
		inf->thumbname = "bmi";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c1a0", 4))
	{
		if (!stricmp(mapname, "c1a0c"))
		{
			inf->chpname = "Unforseen Consequences";
			inf->thumbname = "uc";
		}
		else
		{
			inf->chpname = "Anomalous Materials";
			inf->thumbname = "am";
		}
		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c1a1", 4))
	{
		inf->chpname = "Unforseen Consequences";
		inf->thumbname = "uc";
	...
	else if (!strncmp(mapname, "c1a2", 4))
	{
		inf->chpname = "Office Complex";
		inf->thumbname = "oc";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c1a3", 4))
	{
		inf->chpname = "''We've Got Hostiles''";
		inf->thumbname = "wgh";
	...
	else if (!strncmp(mapname, "c1a4", 4))
	{
		inf->chpname = "Blast Pit";
		inf->thumbname = "bp";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c2a1", 4))
	{
		inf->chpname = "Power Up";
		inf->thumbname = "pu";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c2a2", 4))
	{
		inf->chpname = "On A Rail";
		inf->thumbname = "oar";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c2a3", 4))
	{
		inf->chpname = "Apprehension";
		inf->thumbname = "appr";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c2a4", 4))
	{
		if (!stricmp(mapname, "c2a4d") ||
			!stricmp(mapname, "c2a4e") ||
			!stricmp(mapname, "c2a4f") ||
			!stricmp(mapname, "c2a4g"))
		{
			inf->chpname = "Questionable Ethics";
			inf->thumbname = "qe";
		}
		else
		{
			inf->chpname = "Residue Processing";
			inf->thumbname = "rp";
		}
		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c2a5", 4))
	{
		inf->chpname = "Surface Tension";
		inf->thumbname = "st";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c3a1", 4))
	{
		inf->chpname = "''Forget About Freeman!''";
		inf->thumbname = "faf";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c3a2", 4))
	{
		inf->chpname = "Lambda Core";
		inf->thumbname = "lc";

		inf->chaptertitles = 1;
	}
	else if (!stricmp(mapname, "c4a1"))
	{
		inf->chpname = "Xen";
		inf->thumbname = "xen";
	...
	else if (!strncmp(mapname, "c4a1", 4))
	{
		inf->chpname = "Interloper";
		inf->thumbname = "ilop";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c4a2", 4))
	{
		inf->chpname = "Gonarch's Lair";
		inf->thumbname = "gl";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c4a3", 4))
	{
		inf->chpname = "Nihilanth";
		inf->thumbname = "nihil";

		inf->chaptertitles = 1;
	}
	else if (!strncmp(mapname, "c5a1", 4))
	{
		inf->chpname = "Epilogue";
		inf->thumbname = "epil";

		inf->chaptertitles = 1;
	}

	inf->gamemode = "Campaign";
}